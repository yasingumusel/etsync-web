...
export default function SetupSyncWizard({
  ...
  useEffect(() => {
    ...
    (async () => {
      const [listingsRes, selectionRes] =
        variant === "manage"
          ? await Promise.all([
...
function Centered({ children, wide }: { children: React.ReactNode; wide?: boolean }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-grid px-6 py-12">
...
function Card({ children, wide }: { children: React.ReactNode; wide?: boolean }) {
  return (
    <div className={`card-glass rounded-2xl p-8 ${wide ? "" : "text-center"}`}>{children}</div>
  );
}
